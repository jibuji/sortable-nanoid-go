...
func New(cfg Config) (*Generator, error) {
	...
	timestampLength := calculateTimestampLength(len(cfg.Alphabet), cfg.TimestampLevel)
	...
	g.fillCharPool()
	g.maxAllowedTime = g.timestampStart.Add(time.Duration(1<<63 - 1))
	return g, nil
}
...
func (g *Generator) Decode(id string) (time.Time, string, string, error) {
	...
	date := g.timestampStart.Add(getTimestampDuration(g.timestampLevel) * time.Duration(timestamp))
	return date, chronoPart, randomPart, nil
}

func (g *Generator) getTimespan(t time.Time) int64 {
	duration := t.Sub(g.timestampStart)
	return int64(duration / getTimestampDuration(g.timestampLevel))
}

func getTimestampDuration(timestampLevel TimestampLevel) time.Duration {
...
func (g *Generator) GetMaxDate() time.Time {
	duration := getTimestampDuration(g.timestampLevel) * time.Duration(g.maxTimestamp)
...
func calculateTimestampLength(base int, level TimestampLevel) int {
	// Calculate minimum length needed based on level
	var unitsPerYear int64
	switch level {
	case Nanosecond:
		unitsPerYear = 365 * 24 * 60 * 60 * 1000 * 1000 * 1000
	case Microsecond:
		unitsPerYear = 365 * 24 * 60 * 60 * 1000 * 1000
	case Millisecond:
		unitsPerYear = 365 * 24 * 60 * 60 * 1000
	case Second:
		unitsPerYear = 365 * 24 * 60 * 60
	case Minute:
		unitsPerYear = 365 * 24 * 60
	case Hour:
		unitsPerYear = 365 * 24
	case Day:
		unitsPerYear = 365
	...
	// Calculate for 100 years by default
	units := unitsPerYear * 200
